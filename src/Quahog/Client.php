...
class Client
{
	 ...
	 * 
	 * @param bool  $isStat  Is the command a STATS command?
     *
     * @param string $command
     */
    private function _sendCommand($command, $isStat = false)
    {
    	// Prepend 'n' to the command when in session (but not for STATS command)
    	$command = ($this->_inSession && !$isStat) ? 'n'. $command : $command;
